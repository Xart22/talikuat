<?php
	include_once('Model.php');
	
	Class ItemUnor  extends Database
	{
		public function __construct()
		{
			parent:: __construct();
		}
		
		public function itemUnor()
		{
			$itemUnor = array();
			$itemUnorQuery = mysqli_query($this->conn,"SELECT * FROM kantor");
			while($itemUnorQueryFetch = mysqli_fetch_assoc($itemUnorQuery))
			{
				$itemUnor[] = $itemUnorQueryFetch;
			}
			return $itemUnor;
		}
	/*	
		public function maxInvoiceId()
		{
			$maxInvoiceIdQuery = mysqli_query($this->conn, "SELECT max(id)+1 AS maxInvoiceId FROM invoice_details");
			$maxInvoiceId = mysqli_fetch_assoc($maxInvoiceIdQuery);
			return $maxInvoiceId;
		}
		
		public function vat()
		{
			$vatQ = mysqli_query($this->conn, "SELECT* FROM vat");
			$vat = mysqli_fetch_assoc($vatQ);
			return $vat;
		}
		*/
	}
		
?>